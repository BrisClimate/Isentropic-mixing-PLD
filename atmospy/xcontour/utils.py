...
deg2earth = 2.0 * np.pi * rearth / 360.0
deg2mars  = 2.0 * np.pi * rmars  / 360.0
...
def get_planet_parameters(planet):
    if planet == 'earth':
        r = rearth
        deg2m = deg2earth
        g = ge
        ...
        g = gm
        omega = omega_M
    return r, deg2m, g, omega

def add_latlon_metrics(dset, dims=None, boundary=None,deg2m=deg2earth):
    """
    ...
    if 'X' in periodic:
        dlonC = grid.diff(lonC, 'X')#, boundary={'Y': 'extend'})#boundary_discontinuity=360)
        dlonG = grid.diff(lonG, 'X')#, boundary={'Y': 'extend'})#boundary_discontinuity=360)
    else:
        dlonC = grid.diff(lonC, 'X', boundary='extrapolate')
        dlonG = grid.diff(lonG, 'X', boundary='extrapolate')
    
    dlatC = grid.diff(latC, 'Y', boundary='extend')
    dlatG = grid.diff(latG, 'Y', boundary='extend')
    
    coords['dxG'], coords['dyG'] = __dll_dist(dlonG, dlatG, lonG, latG,deg2m=deg2m)
    coords['dxC'], coords['dyC'] = __dll_dist(dlonC, dlatC, lonC, latC,deg2m=deg2m)
...
def equivalent_latitudes(areas,r=rearth):
    """
...
def latitude_lengths_at(lats,r=rearth):
    """
    ...
    """
    Lmin = (2.0 * np.pi * r * np.cos(np.deg2rad(lats))).astype(lats.dtype)
...
"""
def __dll_dist(dlon, dlat, lon, lat, deg2m=deg2earth):
    """
